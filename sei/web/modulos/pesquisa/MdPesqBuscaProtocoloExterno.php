	...
	public static function executar($q, $strDescricaoPesquisa, $strObservacaoPesquisa, $inicio, $numMaxResultados, $strParticipanteSolr, $md5Captcha = null) {
		...
		if (in_array("P", $checkbox)) {
			...
			if($bolPesquisaProcessoRestrito){
				...
				array_push($grupo, "(sta_prot:P AND tipo_aces:P)");
			}
		...
		if ($itens == 0){
  		...
  		for ($i = 0; $i < $numRegistros; $i++) {
			...
			$regResultado = $registros[$i];
  			...
  			$dados["tipo_acesso"] = SolrUtil::obterTag($regResultado, 'tipo_aces', 'str');
  			$dados["id_unidade_acesso"] = SolrUtil::obterTag($regResultado, 'id_uni_aces', 'str');
  			$dados["id_unidade_geradora"] = SolrUtil::obterTag($regResultado, 'id_uni_ger', 'int');
  			...
  			$dados["protocolo_documento_formatado"] = SolrUtil::obterTag($regResultado, 'prot_doc', 'str');
  			$dados["protocolo_processo_formatado"] = SolrUtil::obterTag($regResultado, 'prot_proc', 'str');
  			$dados["id_usuario_gerador"] = SolrUtil::obterTag($regResultado, 'id_usu_ger', 'int');
  			$dados['id_tipo_processo'] = SolrUtil::obterTag($regResultado, 'id_tipo_proc', 'int');
  			$dados["identificacao_protocolo"] = SolrUtil::obterTag($regResultado, 'numero', 'str');
  			$dados["descricao_protocolo"] = SolrUtil::obterTag($regResultado, 'desc', 'str');
  			...
  			$dtaGeracao = SolrUtil::obterTag($regResultado, 'dta_ger', 'date');
  			...
  			// PROTOCOLO
  			$idProtocolo = SolrUtil::obterTag($regResultado, 'id_prot', 'long');
  			...
  			$tituloCompleto .= "<img border=\"0\" src=\"solr/img/arvore.png\" alt=\"\" title=\"Visualizar árvore\" width=\"14\" height=\"16\" class=\"arvore\" />";
  			$tituloCompleto .= "</a>";
