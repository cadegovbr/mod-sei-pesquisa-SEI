...
//   InfraDebug::getInstance()->limpar();
  ...
  $arrMdPesqTipoProcessoDTO = new MdPesqTipoProcessoDTO();
  $arrMdPesqTipoProcessoDTO->retNumId();
  $arrMdPesqTipoProcessoDTO->retDtaInicio();
  $arrMdPesqTipoProcessoDTO->retDtaFim();

  $objPesquisaTipoProcessoRN = new MdPesqTipoProcessoRN();
  $arrMdPesqTipoProcessoDTO = $objPesquisaTipoProcessoRN->listar($arrMdPesqTipoProcessoDTO);

  foreach ($arrMdPesqTipoProcessoDTO as $tipoProcessoDTO){
    if ($tipoProcessoDTO->getNumId() == $objProcedimentoDTO->getNumIdTipoProcedimento() &&
      (InfraString::isBolVazia($tipoProcessoDTO->getDtaInicio()) || InfraData::compararDatas($tipoProcessoDTO->getDtaInicio(), $objProcedimentoDTO->getDtaGeracaoProtocolo()) > 0) &&
      (InfraString::isBolVazia($tipoProcessoDTO->getDtaFim()) || InfraData::compararDatas($tipoProcessoDTO->getDtaFim(), $objProcedimentoDTO->getDtaGeracaoProtocolo()) < 0)){
  ...
  foreach($arrObjRelProtocoloProtocoloDTO as $objRelProtocoloProtocoloDTO){
    ...
    if ($objRelProtocoloProtocoloDTO->getStrStaAssociacao()==RelProtocoloProtocoloRN::$TA_DOCUMENTO_ASSOCIADO){
      ...
      if ($objDocumentoRN->verificarSelecaoAcessoExterno($objDocumentoDTO)){
        ...
        if (($bolListaDocumentoProcessoPublico && $objProtocoloPesquisaPublicaDTO->getObjDocumentoDTO()->getStrStaNivelAcessoLocalProtocolo() == ProtocoloRN::$NA_PUBLICO) ||
          ($bolListaDocumentoProcessoRestrito && $objProtocoloPesquisaPublicaDTO->getObjDocumentoDTO()->getStrStaNivelAcessoLocalProtocolo() == ProtocoloRN::$NA_RESTRITO)
        ) {
